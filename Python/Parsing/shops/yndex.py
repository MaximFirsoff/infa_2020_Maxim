...
def getallcookies():
    ...
    """
    cookieslist = browsercookie.firefox()  # get cookies from firefox
    cookieslist2 = str(cookieslist).replace('<CookieJar[<Cookie ', '')  # replace sting
    cookieslist2 = str(cookieslist2).replace('>]>', '')  # replace sting
    cookieslist2 = cookieslist2.split('>, <Cookie ')  # spliting by '>, <Cookie '
    # spliting by '=", " for ' and "/" to get for each element
    cookieslist2 = list(map(lambda x: re.split('=| for |/', x), cookieslist2))
    return cookieslist2
